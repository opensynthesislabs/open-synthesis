...
The synthesis layer uses Llama 3.1 70B Instruct with directional ablation applied via [Heretic](https://github.com/p-e-w/heretic) to remove refusal behavior on scientific topics. The ablated model is available at [`opensynthesis/Llama-3.1-70B-heretic-lora`](https://huggingface.co/opensynthesis/Llama-3.1-70B-heretic-lora). Use of this model requires acceptance of the [Llama 3.1 Community License](https://github.com/meta-llama/llama-models/blob/main/models/llama3_1/LICENSE).
...
| Ablated model | [`opensynthesis/Llama-3.1-70B-heretic-lora`](https://huggingface.co/opensynthesis/Llama-3.1-70B-heretic-lora) |
| Architecture | `LlamaForCausalLM` |
| Parameters | 70B |
| Method | Heretic directional ablation (Arditi et al., 2024) |
| Optimization | 200 Optuna TPE trials (9/100 refusals, KL 0.0938) |
| Context window | 128K tokens (native, fully usable) |
| Serving | AWQ 4-bit quantization, tensor parallel across 2x A100 80GB |
...
Deploy on a RunPod GPU pod with [vLLM](https://github.com/vllm-project/vllm). Download the ablated model, quantize to AWQ, and serve:

```bash
# On a RunPod GPU pod (2x A100 80GB, ~$2.38/hr):
pip install vllm autoawq huggingface_hub
# Download and quantize (see runpod_deployment.md for full instructions)
